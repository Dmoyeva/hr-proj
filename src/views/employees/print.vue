...
<script>
import { getStuffPersonalInfo, getStuffJobInfo } from '@/api/employee'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getStuffPersonalInfo() : this.getStuffJobInfo()
  },
  // 组件更新
  methods: {
    async getStuffPersonalInfo() {
      this.formData = await getStuffPersonalInfo(this.userId) // 获取个人基本信息
    },
    async getStuffJobInfo() {
      const userInfo = await getUserinfoDetail(this.userId)
      const jobInfo = await getStuffJobInfo(this.userId) // 获取个人基本信息
