<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="updateStuffJobInfo">保存更新</el-button>
...
import { getStuffJobInfo, updateStuffJobInfo, getBriefStuffInfo } from '@/api/employee'
...
export default {
  ...
  created() {
    this.getStuffJobInfo()
    this.getBriefStuffInfo()
  },
  methods: {
    async getStuffJobInfo() {
      this.formData = await getStuffJobInfo(this.userId)
    },
    async updateStuffJobInfo() {
      await updateStuffJobInfo(this.formData)
      this.$message.success('更新成功')
    },
    async getBriefStuffInfo() {
      this.depts = await getBriefStuffInfo()
    }
  }
}
</script>
